...
const MIN_NOTES = 140
const MAX_NOTES = 720
const EDGE_TRIM_SAMPLES = 30
const MIN_COMPLEX_SAMPLES = EDGE_TRIM_SAMPLES * 2 + 2
...
const centralSecondDifference = (samples: ComplexSample[]) => {
  const differences: ComplexSample[] = []

  for (
    let index = EDGE_TRIM_SAMPLES;
    index < samples.length - EDGE_TRIM_SAMPLES;
    index += 1
  ) {
    differences.push({
      re: samples[index + 1].re - 2 * samples[index].re + samples[index - 1].re,
      im: samples[index + 1].im - 2 * samples[index].im + samples[index - 1].im,
    })
  }
...
export const parseGwCsv = (
  ...
  const strainSamples = parseComplexSamples(text)

  if (strainSamples.length < MIN_COMPLEX_SAMPLES) {
    throw new Error(`GW CSV needs at least ${MIN_COMPLEX_SAMPLES} complex samples.`)
  }

  const visibleStrainSamples = strainSamples.slice(
    EDGE_TRIM_SAMPLES,
    -EDGE_TRIM_SAMPLES,
  )
  const hPlus = visibleStrainSamples.map((sample) => sample.re)
  const hCross = visibleStrainSamples.map((sample) => -sample.im)
  const samples = centralSecondDifference(strainSamples)
  const ddotHPlus = samples.map((sample) => sample.re)
  const ddotHCross = samples.map((sample) => -sample.im)
  ...
  const plusVelocity = normalizeLinear(
    samples.map((_, index) => Math.abs(ddotHPlus[index])),
  )
  const crossVelocity = normalizeLinear(
    samples.map((_, index) => Math.abs(ddotHCross[index])),
  )
  ...
  return {
    ...
    gwWaveform: {
      sampleCount: visibleStrainSamples.length,
